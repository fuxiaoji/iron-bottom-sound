...
## 2026-09-24（续）：接入智谱免费档 glm-4.5-flash，并把「每个 agent 的思考」搬进界面

- **需求（用户）**：「在本地启动并嵌入glm免费模型我测试一下，开启了测试模式后可以看见每个agent思考」。
- **探测（密钥只从 `ZHIPU_API_KEY` 环境变量读，未写进任何文件）**：`glm-5.2`/`glm-4.6` 返回 1113
  「余额不足或无可用资源包」；免费可用的是 **`glm-4.5-flash`** 与 `glm-4-flash`。
  二者都返回严格 JSON，但**只有 `glm-4.5-flash` 在 `thinking:{type:enabled}` 下真的返回
  `reasoning_content`**（实测一次 964 字推理 + 353 字 JSON，22s）；`glm-4-flash` 不是推理模型，
  开了思考链也没有推理内容。因此接入选择 = `glm-4.5-flash` + 思考链开 + `max_tokens=3000`
  （推理先花预算：上限太小会「只思考不回答」，适配层会正确地拒绝解析）。
- **后端**：`CommandDelayPolicyRequest` 增加 `max_tokens`（默认 3000）；策略视图增加
  `thinking_enabled` / `max_tokens`（直接读活策略对象，界面不自己记一份）。
  **修一个真缺陷**：`fleet` 标志此前只影响返回值，舰队层代理实际上**无条件注册** ——
  于是玩家自己那一侧也会被模型下令。现在 `fleet` 为显式开关，撤销（无密钥）时两层一起清；
  新增回归测试 `test_a_fleet_agent_is_opt_in_only`。
- **前端**：`编队代理记录（调试 · 含思考链）`——每次模型往返分开显示「思考过程」（模型自己的推理，
  标注不参与裁决）与「原始回复」，并显示 `finish_reason` 与输出 token 数；重试/被拒往返同样可见；
  舰队层代理条目单独标「舰队层代理」并列出它本回合下的令或 `no_order`；
  模型卡显示「思考链：开/关」「输出上限 N tokens」并说明如何看到思考。
  另：命令延迟模式下推进时加忙碌遮罩（6–7 次模型往返本地实测约 5 分钟，此前点了没反应）。
- **实测（用户对局未被改动）**：独立进程草稿对局 5/5 条目带思考链（1.2k–4.6k 字）、全部被采纳、
  0 次回退教条；**服务进程**草稿对局同样 5/5 带思考链（含同盟舰队代理 1928 字推理）。
  界面截图核对：模型徽标、思考块、原始回复、决策、火力权重、备忘、被拒重试全部按预期渲染。
  用户的 `aecaa4f7` 全程 `updated_at` 未变（09-22 11:49）。
- **如实记下的两个坑**：① **策略注册表是进程级的**：新建任何命令延迟对局都会把编队策略重置为
  教条（`create_game` 里那次注册），因此「接入模型」在新建对局后要重做 —— 本次验证中就发生过一次，
  已重新接入；② 本地每次「交接」要等 6–7 次模型往返（开启思考链时每次约 1 分钟）。
- 演示用对局（已跑过模型、可直接看思考链）：`4d9c33b1-e709-439e-8159-a9d66d4249f0`（轴心视角）。

## 2026-09-24（续二）：机动方案"被改了"的修复 —— 显示与提交不同源 + 代理读不懂记号

- **用户报告**：「为什么从移动阶段进入鱼雷阶段把移动计划改了，本来之前的移动计划是符合我的命令的」
  （对局 `907eb4d5`，二马 EM-01 T2）。
- **先证伪"引擎改了计划"**：T2 已封存的机动批次与三条代理决策逐条一致
  （`axis-battle-line=1SS1S1`、`axis-cruiser-line=1SS1S2`、`axis-destroyer-line=1S3`），
  事件流每方每阶段只记录一次 `orders_submitted`，机动当时尚未结算（舰船仍在 J3/I3/H2…）。
- **真因一（显示与提交不同源）**：机动阶段的计划表与地图画的是 `/suggested-orders` 的建议草稿；
  命令延迟模式下该路径会把代理选的方案**改写成"等价消耗的直行方案"**（`realistic_command.py` 两处：
  含 120° 时替换为 `str(cost)`、超出共同航速区间时替换为 `str(maximum)`），而交接提交的是代理方案原文。
  把该局副本倒回 T2 重算：草稿 `4 / 5 / 1S3`，执行 `1SS1S1 / 1SS1S2 / 1S3` —— 玩家先看到的是直行版。
  **修法**：代理自己的方案对这两处改写免役；交接提交**就是计划表里那份 JSON**（命令延迟模式下它正是
  代理方案：引擎只会在两支编队的航迹真冲突时按既有规则调整一支的航速），因此"表里显示的＝提交的＝执行的"。
- **真因二（代理读不懂记号）**：`1SS1S2` 的 `SS` 是右转 120°，该代理选了掉头方案却在理由里写"保持航向2、
  继续直线前进"。提示词只有 `final_headings`，而那是**目的格可到达的航向集合**（纯直行 `5` 也报 [1,2,3]）。
  **修法**：每条合法机动现在自带引擎算好的 `manoeuvre`（中文逐步）、`ends_heading`、`heading_change_steps`、
  `keeps_heading`、`jams_spaced_column`；`formation_llm.INSTRUCTION` 写明记号读法（S/P/SS/PP）与挤停风险；
  玩家规则文档新增「机动方案的记号」；界面在方案与决策行显示同样的说明，含 120° 的方案给警告
  （旧决策记录用显示层解码兜底，引擎字段存在时以引擎为准）。
- **附带修掉的两个真问题**：
  ① **枚举器给出的方案未必是编队能执行的**：实测出现 `speed 5 is outside member limits` 与
     `cannot follow guide trail` —— 前者此前靠显示路径把方案夹到 maximum 掩盖。现在枚举时按引擎自己的
     判据过滤：方案速度必须落在全队共同航速区间内；`expand_movement_orders` 的失败仍如实报错。
  ② **跨侧优先级泄漏**：`mode.local_directives` 是全游戏一本账，而 `auto_gunnery` 把两侧条目一起喂进选择器；
     指令按 target_class 也能匹配，于是**对方**代理的偏好会改**我方**选靶。现按编队归属过滤
     （`target_priority.local_directives_for`），并补了会失败的判据
     `test_6_another_sides_local_priority_never_reaches_this_selector`。
- **测试**：新增 `tests/test_command_delay_plan_legibility_v24.py` 6 项；`test_command_delay_gunnery_authority_v23.py`
  增补第 6 项；`test_command_delay_agents_and_memory.py::...never_cross_sides` 改为断言"选择器可见集合"；
  `test_command_delay_formation_agent.py` 的选靶判据改为在固定种子上搜索"一舰双目标"局面（找不到即失败，
  不再默认 PASS）。**黄金基线**：7 行 Classic/Realistic 逐字节零漂移；`cd_s01` 按流程重冻结并记录归因
  （原因：代理方案原样提交/显示 + 跨侧泄漏修复；`record_refreeze.py` 留下的 7 行摘要哈希即证明）。
  `--probe-hash-seeds 0,1,2,8` 稳定。
- **登记未改语义的两项**（等 PI 裁决）：① 编队内"同脉冲同格"（含原地 120° 的方案）在校验/展开路径被接受，
  结算时才以急停收场，而状态机路径靠偏好避开 —— 是否改为拒绝属冻结语义变更；② 两支编队航迹冲突时，
  引擎仍按既有规则"给一支减速"，此时计划表里显示的是调整后的方案、代理原始选择在记录里可查。
- **界面核对**（该局实际页面）：方案行显示「轴心第1编队 机动 1SS1S1（前进 1 格 → 右转 120°（原地调头一步）
  → 前进 1 格 → 右转 60° → 前进 1 格，会挤停纵队）」，决策行与警告块同步。
