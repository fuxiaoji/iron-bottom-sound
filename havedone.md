...
- **根因与修复**：6.1.8 世界平移过去只改鱼雷当前位置，没有同步改 `launch_position` 和 `traversed_hexes`；SVG 因而把两个坐标系的点连成折线。现在当前位置、发射锚点和仍在新纸图内的历史格同步平移，越出新视窗的纯历史尾迹只从显示中裁去。
...
- **验收**：定向引擎/战报/API/移动轨迹测试通过；全量 **353 passed**；TypeScript `tsc -b` 通过，Vite 生产构建通过（40 modules transformed）；`git diff --check` 通过。浏览器插件因本机 localhost URL 安全策略拒绝接管现有标签页，未以绕过方式继续，地图数据层回归由共线/相邻格断言覆盖。
