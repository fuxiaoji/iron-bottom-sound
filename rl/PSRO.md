...
本训练器的想定 1、想定 3、二马（IBS-S-EM-01）全部开启真实模式；二马仍是主要验收槽位，另外两个想定用于真实编队策略的跨场景防过拟合。经典模式只做独立回归，不进入适应度。规则引擎始终独立裁决；训练器只能提交合法编队层订单。
...
~~~powershell
$env:PYTHONPATH = 'backend/src;.'
python -m rl.psro --out rl/results/psro-realistic-v1 --serve-dashboard --dashboard-port 8765
~~~
...
$env:PYTHONPATH = 'backend/src;.'
New-Item -ItemType Directory -Force 'rl/results/psro-realistic-v1' | Out-Null
Start-Process -FilePath 'python' -ArgumentList @('-m','rl.psro','--out','rl/results/psro-realistic-v1','--serve-dashboard','--dashboard-port','8765') -WorkingDirectory $root -RedirectStandardOutput 'rl/results/psro-realistic-v1/stdout.log' -RedirectStandardError 'rl/results/psro-realistic-v1/stderr.log' -WindowStyle Hidden
~~~
...
~~~powershell
$env:PYTHONPATH = 'backend/src;.'
python -m rl.psro --out rl/results/psro-realistic-v1 --resume --serve-dashboard --dashboard-port 8765
~~~
