...
refinement.  Fourth, against best-response resistance, no favorable
...
\noindent\textbf{Players are formations.}  Throughout, each player
$i\in\{B,R\}$ is a moving \emph{formation} --- a rigid or quasi-rigid
line-ahead column of vessels --- not an individual vessel.  The formation's
reference state is $s_i=(c_i,\psi_i,v_i,\chi_i)$: reference point
$c_i=(x_i,y_i)$, formation heading $\psi_i$, speed $v_i$, and capability
state $\chi_i$ (weapon availability).  Vessel $k$ of formation $i$ occupies
\begin{equation}
x_{ik}(t) \;=\; c_i(t) \;+\; R(\psi_i(t))\, p_{ik},
\label{eq:formation}
\end{equation}
where $p_{ik}$ is vessel $k$'s offset in the formation frame ($p_{ik}=(0,
-\ell k)$ for a line-ahead column with astern spacing $\ell$) and
$R(\cdot)$ is the rotation matrix.  The reference point obeys nonholonomic
kinematics, $\dot x_i=v_i\cos\psi_i$, $\dot y_i=v_i\sin\psi_i$,
$\dot\psi_i=\omega_i$, with $v\le\bar v_i$ and $|\omega_i|\le\bar\omega$ (a
...
rotating the velocity vector --- the defining nonholonomic cost.  Reduced
implementations that track a single reference state are read as
\emph{formation reference states} under this definition; no experiment in
this paper supports a claim about isolated single vessels.

\noindent\textbf{Anisotropic formation field.}  Each vessel projects an
interaction kernel $K_{ik}(x,t)=K(r,\theta,\psi_i,\psi_j,\chi_i)$ depending
on range, relative bearing, both headings, and capability state; the
formation field aggregates over its vessels,
\begin{equation}
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.9\textwidth]{figures/fig1_schematic.png}
\caption{Two line-ahead formations compete over future trajectories inside a
directional fire field (background, computed from the calibrated kernel) and a
delayed torpedo hazard corridor.  Solid, dashed, and dotted arcs are candidate
open-loop trajectories; the payoff of a plan is the exposure integrated along
the whole path, not the value at its best instant.}
\label{fig:schematic}
\end{figure}
...
commitment.  Each tile is one $(\etav,\etar)$ cell and the printed number is
the midpoint of the certified interval
$[\underline V_6-\overline V_1,\ \overline V_6-\underline V_1]$.
Certification is encoded separately from magnitude: solid blue (red) borders
are exact sign-certified positive (negative), grey hatching marks an interval
crossing zero, a black cross marks a Grid-5/Grid-7 sign disagreement, and a
small open circle marks a Grid-7 cell not run.}
