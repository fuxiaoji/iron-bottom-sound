...
export interface ShipCombatEntry{sequence:number;turn:number;phase:Phase;direction:"inflicted"|"received";event_type:string;message:string;related_ship_id:string|null;related_ship_name:string|null;rule:Event["rule"];dice:Event["dice"]}
export interface Ship{id:string;name:string;side:Side;ship_type:string;position:HexCoord|null;heading:number;current_speed:number;hull:number|null;max_hull:number|null;fire_markers:number;fired:boolean;sunk:boolean;asset:string|null;max_speed:number|null;min_legal_speed:number|null;max_legal_speed:number|null;torpedo_type:string|null;gun_mounts:GunMount[];torpedo_launchers:TorpedoLauncher[];combat_history:ShipCombatEntry[]}
export interface TorpedoTrack{id:string;side:Side;launcher_ship_id:string;torpedo_type:string;position:HexCoord;heading:number;range_remaining:number;distance_travelled:number;salvo_size:number;contact_ship_ids:string[];hidden:boolean}
