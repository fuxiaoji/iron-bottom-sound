...
export default function App(){
 ...
 const createGame=(scenarioId:string,seed:number,nextMode:"hotseat"|"tutorial"|"vs_ai"|"llm"="hotseat",profile?:string,report=true,key?:string|null,consent?:Parameters<typeof createGameApi>[6],config?:LLMConnectionConfig|null,realisticOverride?:boolean,tutorialScript?:"classic_night"|"erma_grand_fleet")=>createGameApi(scenarioId,seed,nextMode,profile,report,key,consent,config,realisticOverride??realisticCommand,tutorialScript);
 ...
 useEffect(()=>{
  const revealPanel=(event:Event)=>{const panel=(event as CustomEvent<"fleet"|"map"|"orders">).detail;if(panel==="fleet")setFleetPanelCollapsed(false);if(panel==="orders")setOrdersPanelCollapsed(false)};
 ...
 const start=async(scenario_id:string,nextMode:"hotseat"|"tutorial"|"vs_ai"|"llm"="hotseat",playerSide:Side="axis",profile="balanced",realisticOverride?:boolean,tutorialScript?:"classic_night"|"erma_grand_fleet")=>{try{if(nextMode==="llm"&&!llmModel.trim())throw new Error("请先填写模型名称");const g=await createGame(scenario_id,1,nextMode,profile,recordReport,llmKey||null,{allow:researchAllow,handle:researchHandle||null},nextMode==="llm"?llmConfig:null,realisticOverride,tutorialScript);setGame(g.game_id);setMode(nextMode);setAiProfile(profile);setSide(playerSide);if(realisticOverride!==undefined)setRealisticCommand(realisticOverride);setLlmBusy(false);setServerReport(undefined);setReportOpen(false);setReportEnabled(recordReport);setMoveEditorShip(null);setTorpedoAssistOverlay(null);setPlannedTrajectories(null);setFireHeatmapMode("off");setFireHeatmap(null);setDebug(false);setView(await viewGame(g.game_id,playerSide,false));setError("")}catch(e){setError(String(e))}};
