...
import {advance,createGame,gameEvents,handoff,submitOrders,suggestedOrders,tutorialOpponent,viewGame} from "./api";
...
export default function App(){
 const [game,setGame]=useState<string>();const [view,setView]=useState<Observation>();const [side,setSide]=useState<Side>("axis");const [mode,setMode]=useState<"hotseat"|"tutorial">("hotseat");const [locked,setLocked]=useState(false);const [selected,setSelected]=useState<Ship>();const [error,setError]=useState("");const [draft,setDraft]=useState("");const [planIntent,setPlanIntent]=useState("");const [battleReport,setBattleReport]=useState<TurnBattleReport>();const [reportOpen,setReportOpen]=useState(false);
 ...
 const clearAndLock=async()=>{if(game)await handoff(game);setView(undefined);setSelected(undefined);setDraft("");setPlanIntent("");setBattleReport(undefined);setReportOpen(false);setLocked(true)};
