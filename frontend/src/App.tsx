...
import {advance,createGame,handoff,submitOrders,suggestedOrders,tutorialOpponent,viewGame} from "./api";
...
export default function App(){
 const [game,setGame]=useState<string>();const [view,setView]=useState<Observation>();const [side,setSide]=useState<Side>("axis");const [mode,setMode]=useState<"hotseat"|"tutorial">("hotseat");const [locked,setLocked]=useState(false);const [selected,setSelected]=useState<Ship>();const [error,setError]=useState("");const [draft,setDraft]=useState("");const [planIntent,setPlanIntent]=useState("");
 const refresh=async(id=game,s=side)=>{if(id)setView(await viewGame(id,s))};
 useEffect(()=>{let active=true;setPlanIntent("");if(view&&game&&orderPhases.has(view.phase)){suggestedOrders(game,side).then(batch=>{if(active)setDraft(JSON.stringify(batch,null,2))}).catch(()=>{if(active)setDraft(JSON.stringify(defaultBatch(view,side),null,2))})}else setDraft("");return()=>{active=false}},[view,side,game]);
 ...
 const start=async(scenario_id:string,nextMode:"hotseat"|"tutorial"="hotseat")=>{try{const g=await createGame(scenario_id,1,nextMode);setGame(g.game_id);setMode(nextMode);setSide("axis");setView(await viewGame(g.game_id,"axis"));setError("")}catch(e){setError(String(e))}};
 const clearAndLock=async()=>{if(game)await handoff(game);setView(undefined);setSelected(undefined);setDraft("");setPlanIntent("");setLocked(true)};
 const act=async()=>{if(!game||!view)return;try{
  if(orderPhases.has(view.phase)){
   ...
   if(mode==="tutorial"){await tutorialOpponent(game,side);await advance(game);await refresh(game,side)}else{if(result.both_submitted)await advance(game);await clearAndLock()}
 ...
 if(!view)return null;
 return <main className={`game ${mode}`}><header><div><p className="eyebrow">{mode==="tutorial"?"GUIDED TUTORIAL":view.scenario_id}</p><h1>{mode==="tutorial"?"新手教学：通道行动":view.scenario_title}</h1></div><div className="turn">第 {view.turn}/{view.max_turns} 回合<br/><b>{phaseNames[view.phase]}</b></div><button onClick={act}>{orderPhases.has(view.phase)?mode==="tutorial"?"检查本课并继续":"校验、封存并交接":"执行引擎裁决"}</button></header><section className="workspace"><HexMap ships={view.ships} onSelect={setSelected} viewerSide={side} visibility={view.visibility} showVisibility={mode==="tutorial"}/><aside>{mode==="tutorial"&&<TutorialPanel view={view}/>}<ShipStatusCard ship={selected}/>{orderPhases.has(view.phase)&&<section className="order-editor"><h2>本阶段秘密计划表</h2><p>表格修改会同步到完整 OrderBatch；提交时仍由规则引擎严格校验。</p><PlanSheet view={view} side={side} draft={draft} setDraft={setDraft} intent={planIntent} setIntent={setPlanIntent} tutorial={mode==="tutorial"}/></section>}<h2>裁决日志</h2><ol className="events">{[...view.recent_events].reverse().map(e=><li key={e.sequence}><span>T{e.turn} · {phaseNames[e.phase]}</span>{e.message}{e.dice&&<small>{e.dice.notation}: {e.dice.raw}{e.dice.adjusted!==undefined&&` → ${e.dice.adjusted}`}</small>}{e.rule&&<code>{e.rule.rule_id} · p.{e.rule.pdf_page}</code>}</li>)}</ol></aside></section>{error&&<div className="error"><b>还不能继续：</b><br/>{error}</div>}</main>;
}
