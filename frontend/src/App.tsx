...
import {advance,aiOpponent,createGame,fieldOfFire,gameEvents,handoff,legalActions,movementTrajectories,sealedTrajectories,submitOrders,suggestedOrders,tutorialOpponent,viewGame} from "./api";
...
export default function App(){
 const [game,setGame]=useState<string>();const [view,setView]=useState<Observation>();const [side,setSide]=useState<Side>("axis");const [mode,setMode]=useState<"hotseat"|"tutorial"|"vs_ai">("hotseat");const [aiProfile,setAiProfile]=useState("balanced");const [vsScenario,setVsScenario]=useState("IBS-S-03");const [vsSide,setVsSide]=useState<Side>("axis");const [vsProfile,setVsProfile]=useState("balanced");const [locked,setLocked]=useState(false);const [selected,setSelected]=useState<Ship>();const [error,setError]=useState("");const [draft,setDraft]=useState("");const [planIntent,setPlanIntent]=useState("");const [actionHints,setActionHints]=useState<Record<string,unknown>>({});const [battleReport,setBattleReport]=useState<TurnBattleReport>();const [reportOpen,setReportOpen]=useState(false);const [moveEditorShip,setMoveEditorShip]=useState<string|null>(null);const [torpedoAssistOverlay,setTorpedoAssistOverlay]=useState<{path:HexCoord[];intercept:HexCoord|null}|null>(null);const [plannedTrajectories,setPlannedTrajectories]=useState<MovementTrajectory[]|null>(null);const [fireHeatmapMode,setFireHeatmapMode]=useState<FireHeatmapMode>("off");const [fireHeatmap,setFireHeatmap]=useState<FireHeatmapResponse|null>(null);const [debug,setDebug]=useState(false);
 ...
 const start=async(scenario_id:string,nextMode:"hotseat"|"tutorial"|"vs_ai"="hotseat",playerSide:Side="axis",profile="balanced")=>{try{const g=await createGame(scenario_id,1,nextMode,profile);setGame(g.game_id);setMode(nextMode);setAiProfile(profile);setSide(playerSide);setBattleReport(undefined);setReportOpen(false);setMoveEditorShip(null);setTorpedoAssistOverlay(null);setPlannedTrajectories(null);setFireHeatmapMode("off");setFireHeatmap(null);setDebug(false);setView(await viewGame(g.game_id,playerSide,false));setError("")}catch(e){setError(String(e))}};
 ...
 const act=async()=>{if(!game||!view)return;try{
  if(orderPhases.has(view.phase)){
   ...
   if(mode==="tutorial"){await tutorialOpponent(game,side);await advance(game,side);await refresh(game,side)}else if(mode==="vs_ai"){await aiOpponent(game,side,aiProfile);await advance(game,side);await refresh(game,side)}else{if(result.both_submitted)await advance(game,side);await clearAndLock()}
