...
export default function App(){
 ...
 if(!view)return null;
 return <main className={`game ${mode}`}><header><div><p className="eyebrow">{mode==="tutorial"?"GUIDED TUTORIAL":view.scenario_id}</p><h1>{mode==="tutorial"?"新手教学：通道行动":view.scenario_title}</h1></div><div className="turn">第 {view.turn}/{view.max_turns} 回合<br/><b>{phaseNames[view.phase]}</b></div><button onClick={act}>{orderPhases.has(view.phase)?mode==="tutorial"?"校验并让教官行动":"校验、封存并交接":"执行引擎裁决"}</button></header><section className="workspace"><HexMap ships={view.ships} onSelect={setSelected}/><aside>{mode==="tutorial"&&<TutorialPanel view={view}/>}<ShipStatusCard ship={selected}/>{orderPhases.has(view.phase)&&<section className="order-editor"><h2>本阶段秘密计划表</h2><p>表格修改会同步到完整 OrderBatch；提交时仍由规则引擎严格校验。</p><PlanSheet view={view} side={side} draft={draft} setDraft={setDraft} intent={planIntent} setIntent={setPlanIntent}/></section>}<h2>裁决日志</h2><ol className="events">{[...view.recent_events].reverse().map(e=><li key={e.sequence}><span>T{e.turn} · {phaseNames[e.phase]}</span>{e.message}{e.dice&&<small>{e.dice.notation}: {e.dice.raw}{e.dice.adjusted!==undefined&&` → ${e.dice.adjusted}`}</small>}{e.rule&&<code>{e.rule.rule_id} · p.{e.rule.pdf_page}</code>}</li>)}</ol></aside></section>{error&&<div className="error">{error}</div>}</main>;
}
