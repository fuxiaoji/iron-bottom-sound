...
export function StartScreen(props:Props){
 ...
 return <main className="landing start-screen">
  ...
  <section className="llm-settings"><div className="llm-setting-grid">
   <label>接口供应商<select value={props.llmProvider} onChange={event=>{const provider=event.target.value as "deepseek"|"zhipu";props.setLlmProvider(provider);props.setLlmModel(provider==="zhipu"?"glm-5.3-flash":"deepseek-v4-flash")}}><option value="zhipu">智谱 BigModel</option><option value="deepseek">DeepSeek</option></select></label>
   <label>模型<input value={props.llmModel} onChange={event=>props.setLlmModel(event.target.value)}/></label>
  ...
  <label className="vision-toggle"><input type="checkbox" checked={props.llmVision} onChange={event=>props.setLlmVision(event.target.checked)}/>向 LLM 发送本方可见地图</label></section>
  <div className="scenario-grid">
   ...
   <div className="vs-card"><b>人机大战</b><select value={scenario} onChange={event=>setScenario(event.target.value)}>{scenarios.map(([id,label])=><option key={id} value={id}>{label}</option>)}</select><select value={side} onChange={event=>setSide(event.target.value as Side)}><option value="axis">我指挥轴心</option><option value="allies">我指挥同盟</option></select><select value={profile} onChange={event=>setProfile(event.target.value)}><option value="balanced">均衡</option><option value="fleet">大舰队编队</option><option value="line">长纵队</option><option value="cautious">保守</option></select><button onClick={()=>props.onStart(scenario,"vs_ai",side,profile)}>开始</button></div>
