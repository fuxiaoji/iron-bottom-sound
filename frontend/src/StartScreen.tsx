...
export function StartScreen(props:Props){
 ...
 return <main className="landing start-screen">
  ...
  {journey==="learn"?<section className="academy-route">
   ...
   <div className="launch-summary"><div><b>{chosenTitle}</b><span>{commandMode==="command_delay"?"命令延迟（真实编队核心）":effectiveRealistic?"真实编队指挥":"经典逐舰指挥"} · {side==="axis"?"轴心席位":"同盟席位"}</span></div><div className="launch-actions"><button className="quiet briefing-btn" onClick={()=>openBriefing(scenario)}>📄 剧本简报</button><button className="launch" onClick={()=>props.onStart(scenario,matchMode,side,profile,effectiveRealistic,undefined,commandMode==="command_delay")}>{startLabel} →</button></div></div>
   </section>
