...
def _column_labels(columns: int = MAP_COLUMNS) -> list[str]:
...
def _board_cells(
    ...
    """扩展海图 token 网格 + 图例 + 舰→token 映射（呈现层，非规则）。

    render_board 与 PIL 战报渲染共用，保证符号/编号完全一致。全从
    `engine.observe` 可见集派生（战争迷雾一致）。网格尺寸跟随局声明
    （state.map_columns × state.map_rows），大战场整图照常渲染。
    """
    observation = engine.observe(state.game_id, side)
    height = state.map_rows
    width = state.map_columns
    ...
    def put(coord, token: str) -> None:
        ...
        if 0 <= row < height and 0 <= coord.q < width:
...
def render_board(state: GameState, engine: IronBottomEngine, side: Side) -> str:
    """扩展海图 ASCII 棋盘（投影一）。按 label 定位，只画可见集。"""
    ...
    lines = [header]
    for display_row in range(state.map_rows):
