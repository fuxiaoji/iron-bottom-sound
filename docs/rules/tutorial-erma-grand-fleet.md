...
## IBS-TUT-EM-01：远程炮战教学部署

二马原资料允许双方在各自区域自由部署；正式对局默认编队相距约 24 格，超出首轮能见度。教学脚本将轴心全部初始格统一平移 `+4 列、+5 行`，同盟统一平移 `-4 列、-5 行`，保持每方相对队形、舰首和航速不变，使首轮编队完成后进入 15 格内的合法远程炮战。该平移只发生在教程 reset，正式二马部署不变。
