...
## 2026-09-13 v14 有限重规划预算与完整论文（进行中）

用户已批准 MASTER_PLAN：本机 16 GB、单重型进程、8 GB 进程树预算；先协议冻结与求解器验证，再执行开发/冻结测试/稳健性/消融/决策实验，完成理论、完整英文稿、约八图四表、两轮内部审阅和复现包。采用固定公开更新日程与密封块命令，固定对手 F/C 分别报告。旧 v12/v13 结果与稿件保持原样。一区竞争力单独评价，不能由任务完成替代。

首批：保存 research/final_v14/MASTER_PLAN.md 与台账、冻结输入和实验配置；实现任意日程完美回忆序列形式、独立微型策略枚举及日程端点/玩家交换/信息边界测试；通过后进行本机可行性基准。完整实验限时与未分辨处理按 MASTER_PLAN 执行。

当前批次：开发实验保持单进程执行，核心、辅助算法和新增 F 对手区间损失算法均已冻结。完善论文图表生成器、证据注册表、期刊定位和复现入口；补充区间损失定理的主文/附录/证据一致性。继续执行串行开发、测试、稳健性、消融、证书和冷运行比较；随后填入结果、完成整稿及第二轮审阅。任何实现错误按冻结修订流程处理，科学负结果不改变原设计。

## 2026-09-13 v13.1 新实验批次（用户最新范围：直到开写论文前）

状态：已完成当前实验停止分支（未提交）。v13.1 修订后主验证 5/12 正、7/12 强反向，Gate=FAIL；30 项目标回归、所有主实验/能力镜像与冻结文件校验通过。已交付八组 DRAFT 图、20 问答、逐项执行清单、理论/文献/审稿草稿和复现入口。Grid-5/7、转向率、horizon、rigid 扩展按失败分支未触发。v12 稿件制作已被用户新请求暂停；未改标题、摘要、贡献列表或主论文。

实现修订 01：首次验证在 parallel/.75 的独立镜像支付检查失败；定位到 ±150° 射界边界的浮点分类跳变。归档全部初次 v13 输出，固定 1e-8 度边界归并规则，保留原设计并在读取修订后结果前冻结新代码。通过 30 项回归后重算全部 v13，披露前后差异；不得改变原 Gate 或 v12 冻结内容。

1. 创建实验分支，冻结 v12 结论、数据、求解器与现存 fallback 稿件；明确 fallback 并未获二区质量认证。
2. 仅运行 v13 所需回归：矩阵/DP/sequence-form、三种几何、交换、非负性及禁止旧缓存；不重跑 v12 Monte Carlo 审计。
3. 固定对手速度与策略类，完成 Grid-3、T=6、三几何×五自身速度的完整四价值 discovery 和实际交换镜像。当前速度语义为 forced operating speed。
4. 预先明确 favorable-set 域、距离、漂移、reachable correction 与 bounded tracking 指标；保留负 correction 和空集合，修正原公式不能保证 [0,1] 的数学问题。进行冻结探针机制实验与描述性相关/geometry FE 分析。
5. 在读取验证参数结果前冻结确切的 12 个 interaction、数值阈值与 Gate，再运行 held-out 与镜像；非独立 mirror 不增加样本数。
6. 在可计算的小规模上实现真正嵌套的 selectable speed actions 能力对照；不得把按 vmax 缩放的非嵌套有限菜单误称 capability expansion。完成 reduced tracking model 的充分条件证明与反例边界。
7. 仅当精确与机制 Gate 支持时扩展 Grid-5/7、转向率、T=4/6/8 与 rigid 六锚点；失败立即停止实验扩展。
8. 输出 DRAFT 图、结果与证据表、16 项迭代报告、V13_Q1_GATE.md、最终导师 20 问答和复现入口；进行必要核验，更新 havedone.md。无论 Gate 如何，本次停在主论文开写前。

## 2026-09-13 v12 Commitment / Flexibility 论文修复批次

状态：历史实验与否定结论已冻结；写稿部分被用户更新的 v13.1“直到开写论文前”范围暂停，不能标为已写完。附件是待核验研究方案，不能替代模型定义、证据或正确性判断。既有未提交论文与研究文件保留。

1. 冻结 v11 输入、v12 原计划和任务清单；核验并使用 GitHub 科研、审稿、论文与可视化技能。
2. Phase A：完整 LF history cache、交换等变与对称耦合求解、gap 驱动 DO、逐 epoch 误差记录；完成缓存/收敛/对称控制及各 100 个历史状态交换与标量核验。
3. 核验三个所谓 symmetric geometries 是否实际满足交换对称的初态假设；不得强行反对称化非对称博弈或把算法 gap 当反馈策略性能界。
4. 按硬 Gate 执行 Grid-3 精确同 cadence 博弈；依据锚点进入 bilateral 或 flexibility 分支。验证集须在 discovery 前冻结；失败则按计划停止对应实验，不补选参数。
5. 对适用分支实现信息集/sequence form、证明 A–D、discovery/held-out、精细网格与编队稳健性、18-cell 诊断；未执行项注明依赖 Gate，不伪称完成。
6. 在 paper_v12 和 research/final_v12 形成数据驱动图表、完整论文、证据注册表、复现入口、同行评审式质量报告及最终 17 问答；审计所有既有核心论证的表述与证据。
7. 运行必要测试与图表/PDF 视觉验收，追加 havedone.md（含实际测试、证据和基线提交；未提交则明确写明），按证据评估投稿成熟度。
...
1. 在项目根目录增加可双击运行的 `start-game.bat`，自动定位项目目录并调用现有 PowerShell 启动器。
2. 启动成功后沿用现有流程打开浏览器；启动失败时保留窗口，并提示后端、前端错误日志位置。
3. 验证脚本可从非项目工作目录调用，且不会吞掉 PowerShell 的失败退出码。

验收：从项目上级目录直接调用根目录批处理文件，前端与后端分别返回 HTTP 200，PID 文件记录的 Python/Node 进程均存活；`git diff --check` 通过。

## 当前批次：完整裁决日志、炮击目标线与交战结果弹窗（2026-09-04）

状态：已完成（未提交）

1. 对局侧栏不再只显示 `recent_events` 的最后 40 条；通过阵营过滤的事件 API 加载本局完整日志，保留“最近 40 条”切换与面板内滚动。
2. 炮击填单阶段解析当前 `OrderBatch.gunnery`，在地图上为每个己方射手到目标画可辨识辅助线、箭头与舰名标签；只展示玩家已知且已填入的目标。
3. 炮击裁决完成并离开 `gunnery` 阶段后，立即从本次可见裁决事件汇总“我方战果 / 敌方战果”，弹窗显示开火、命中、损伤和击沉摘要；不推测隐藏损伤。
4. 验收覆盖完整日志加载、炮击线随草稿变更、零命中/双方有损伤弹窗、热座/人机/教学流程与手机布局；炮击裁决引擎不变。

验收：TypeScript 与 Vite 生产构建通过；真实浏览器在第 6 回合炮击局显示 16 条目标线和 1496 条阵营可见日志，日志全量/最近 40 条往返正确；在隔离副本中完成双方炮击，弹窗按事件序列正确归组 18 次我方与 57 次敌方齐射，零命中不重复挂载损伤，桌面双栏和 700px 单栏布局均通过。

## 当前批次：可携式存档与阶段复盘（2026-09-04）

状态：已完成（未提交）

目标与边界：

1. 浏览器刷新后自动恢复当前对局；首页列出 SQLite 中已有对局，可从指定阵营继续。
2. 随时导出带版本、完整状态、事件轨迹、阶段快照和自定义想定定义的 `.ibs-save.json`；导入时校验格式、摘要、事件序列和 Pydantic 模型，以新 game_id 安全克隆，不覆盖旧局。
3. 复盘界面按已落库快照查看地图、回合/阶段和截止当时的事件；后端观察仍按所选阵营过滤，不在非调试复盘泄露秘密计划。
4. 导入后可立即续玩；LLM API 密钥继续不进入存档，导入/DAO 不改动规则引擎常量。

验收：后端导出→导入往返、篡改拒绝、快照阵营过滤和旧 SQLite 恢复测试通过；TypeScript/Vite 构建通过；浏览器刷新、首页续玩、下载、导入和复盘交互可用。
...
## 2026-08-30 生产服务器无损同步

状态：已完成（部署提交 `b1f827d`；线上服务 active，旧存档与事件数量未减少）。

1. 只读核对服务器版本、服务状态、数据库路径、对局/事件数量和现有部署清单。
2. 使用 SQLite Backup API 建立带时间戳的线上备份，并记录大小与 SHA-256；不读取、输出或改写 `/etc/tiedi.env`。
3. 增量上传当前 Git 版本的运行源码与前端源码，明确排除 `.git`、SQLite/WAL/SHM、reports、缓存、训练产物、临时文件和密钥。
4. 在服务器构建 `/tiedi/` 前端、执行必要的 Python 导入/迁移检查，随后重启 `tiedi.service`；nginx 配置未变化则不重载 nginx。
5. 验收 systemd active、首页与 API HTTP 200、新前端 bundle、旧对局可读、数据库对局/事件数量不减少，并记录部署提交与回滚路径。

## 2026-08-30 鱼雷 AI 合法性收口与可对战冠军

状态：实施中。保留 `rl/results/psro-realistic-v1` 已有 10,914 局合法结果；失败的 30 个键作为回归证据，不删除数据库、JSONL 或检查点。

1. 修复候选适应度：任何 `ok=false` 对局不得参与阵营反号或适应度平均；候选只要含非法对局即不得入选或注册冠军，面板分别显示合法与非法计数。
2. 从失败键固化真实模式回归，覆盖编队降速、脱队后共同速度群、舵机 60 度限制、舰桥原速强制移动和舵机/舰桥组合绕圈。
3. 规则行为以玩家表特殊损伤条目和 `IBS-R-RC-*` 扩展优先级为依据；不通过放宽校验、失败惩罚或静默停车掩盖非法订单。
4. 聚焦测试和全量回归通过后提交修复，再以 `--resume` 仅重跑失败键并恢复 8765 面板；要求所有历史失败键被合法结果覆盖。
5. PSRO 完成后冻结冠军，运行真实模式想定 1、想定 3、二马的双阵营 fresh-seed 对局及鱼雷战术复盘；自动终局、0 非法命令、0 友军碰撞、0 鱼雷友伤后才宣布“可对战 AI”。

### 2026-08-31 第 0 轮第 3 代强制损伤回归

1. 保留关机前后同一 `psro-realistic-v1` SQLite/JSONL/检查点；不得删除或另开结果库。
2. 固化第 3 代三个精确失败键：阿拉斯加舵损 60° 限制、大和撤退控制器无合法航路、战列线降速后共同速度区间为空。
3. 原版舵/舰桥强制移动优先于真实模式编队跟随；只有地图边界、陆地或同步友舰冲突才允许项目扩展的最后合法脉冲截断，不得用普通 0 MF 掩盖。
4. 三个精确回放、真实模式专项和 PSRO 适应度测试通过后，从第 0 轮第 3 代检查点恢复；新产生 `ok=false` 时立即停止并继续收口。

### 2026-08-31 第 0 轮第 5 代续训合法性收口

状态：正式续训运行中（2026-08-31 14:46 恢复，PID 40732）。继续使用 `rl/results/psro-realistic-v1` 的 10,944 行 SQLite/WAL 账本和原检查点，不删除、不另开结果库，也不让失败对局进入适应度或冠军注册。

1. 从现有 14 个 `ok=false` 结果提取完整任务键、想定、seed、阵营参数和首个裁决错误，优先复现最后停止训练的卷波舵损 120° 转向。
2. 原版舵损、舰桥和强制移动规则仍由引擎权威校验；只修复真实模式状态机的候选生成、编队展开或撤退恢复，不放宽规则、不静默停车、不修改失败账本以伪装通过。
3. 将每类新失败固化为精确回放测试，并运行真实模式、特殊损伤、PSRO 适应度与断点恢复专项；通过后提交代码和测试。
4. 使用原命令 `python -m rl.psro --out rl/results/psro-realistic-v1 --resume --serve-dashboard --dashboard-port 8765` 恢复训练，确认失败键被合法结果覆盖、完成数继续增长、`invalid_games=0`、`stalled=false`。
5. 若续跑再次产生 `ok=false`，训练立即按原机制停止，保留检查点并进入下一轮合法性收口；只有全部三轮完成并通过 fresh-seed 门槛后才注册可对战冠军。

### 手机与平板触控适配

1. 补充 viewport、刘海安全区和 `100dvh`，避免地址栏伸缩、底部手势区及横屏裁切。
2. 小于等于 1100px 时提供固定底部“舰队 / 海图 / 命令”三页签；任一时刻只让一个主工作区接收触控，彻底消除旧版左右栏同时遮住地图的问题。
3. 手机端压缩舰桥标题并将阶段主按钮独占一行；列表、表单、编队、鱼雷、炮击与战报改为单列，按钮和输入控件保持至少 44px 触控高度。
4. 地图保留双指/拖拽能力，缩放、射界与移动编辑控件避开底部导航；强制教练置于导航上方且仍能自动定位当前控件。
5. 使用手机竖屏、手机横屏、平板竖屏和桌面视口检查大厅、二马教学、地图、命令表和战报；构建通过后与最新提交一并无损部署。

## 2026-09-02 舰船素材绑定与自定义剧本模式

状态：进行中。

1. 完成被中断的第2–24页舰卡覆盖审计，确认页面来源、记录数量、重复卡去重和未完成目录项边界。
2. 建立完整战斗记录 ID 到本地棋子素材文件的显式绑定，并为缺图、歧义变体增加阻断校验。
3. 在现有 SQLite 游戏仓储中增加自定义剧本持久化及 CRUD API；自定义剧本只允许完整舰船记录、合法地图布置和合法编队。
4. 在启动页加入自定义剧本编辑器，支持双方选舰、地图摆放、航向速度、经典/真实模式切换和编队旗舰链配置。
5. 扩展一键启动脚本的健康等待与本地冒烟验证，运行后端测试、前端生产构建和浏览器端创建—开局—编队—推进流程。

## 2026-08-30 桌面双侧栏收纳与棋子素材补发

状态：已完成（提交 `0fb22e2`；生产棋子引用 54/54 HTTP 200）。本批只调整布局状态与静态素材部署，不改规则、单位数值或战争迷雾。

1. 桌面宽屏为舰队栏和命令栏提供独立收起/展开按钮；收起后保留可识别的 44px 控制轨，地图立即使用释放出的空间。
2. 手机与平板继续使用“舰队 / 海图 / 命令”底部页签，不叠加桌面收纳按钮；教学控件定位仍可自动打开正确区域。
3. 从结构化想定提取全部棋子引用，逐个核对本地文件与线上 `/tiedi/assets/counters/`；缺失素材按原文件名和 UTF-8 路径补发，不改 YAML 映射。
4. 运行 TypeScript/Vite、教学/API 聚焦测试和桌面折叠交互检查；发布前备份线上数据库、运行文件和棋子目录，发布后验证旧对局、事件数量、全部引用素材及公网 HTTP。

## 2026-09-02 舰船记录全量数据库迁移

状态：进行中。

1. 盘点当前舰船数据结构、来源清单、解析器、存储层和现有测试，确认本批迁移边界。
2. 以舰船记录手册 PDF 和舰船列表 DOCX 为规范来源，逐页/逐表抽取全部记录；抽取结果与原页视觉核对，缺失或歧义字段进入 `docs/rules/open_questions.md`。
3. 将全量舰船记录写入项目现有结构化数据格式，保持稳定 ID、来源页、舰级模板与单舰覆盖字段的兼容性。
4. 增加或更新迁移校验，检查记录总数、名称/ID 唯一性、模板引用、必填字段、来源页与原始资料覆盖率。
5. 运行舰船记录、数据加载、API 导出及相关回归测试，并在 `havedone.md` 记录证据与提交哈希。
