...
1. 补齐炮击口径/目标舰型、舰首舰尾、多目标修正，以及逐炮位摧毁优先级。
2. 完成火灾、沉没漂移、地图边缘整体反向移位和结构化地形/视线阻挡。
3. 为 9.4、9.5、9.8、9.9 补充开/关/组合边界测试；重点审计雷达与隐蔽标记联动的信息边界。
4. 复现所有手册金标范例，将 P3 覆盖矩阵全部提升至 `tested`，再进入 P4 API/UI/持久化。
