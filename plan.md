...
批次结果：移除错误的“左舷仅 A/B、右舷仅 X/Y”约束。规则数据现保存双舷各 `A/B/X/Y` 的相对舰首航向；引擎以舷侧和字母共同生成鱼雷航向，合法行动为每个可用发射器返回四个方向，UI 不再自行分割选项。以舰首 2 为金标，左舷得到 `5/6/1/2`，右舷得到 `5/4/3/2`，规则书青叶“左舷 X”结果为方向 1。全量 `108 passed`，TypeScript 与 Vite 37 模块生产构建通过；重启后的真实 API 已逐舰逐发射器返回四方向。
