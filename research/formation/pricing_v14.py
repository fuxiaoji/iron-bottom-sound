...
def hybrid(A,base,T,S,time_limit=1200.,return_strategies=False):
    """Fixed 64-iteration pricing budget followed by complete sequence form.

    A small engineering pilot showed a highly instance-dependent iteration
    count. This portfolio rule is frozen before held-out experiments. Both
    paths preserve the exact calendar game and report full response bounds.
    """
    started=time.perf_counter()
    r,pol=column_generation(A,base,T,S,time_limit=time_limit,max_iters=64,tolerance=1e-7,return_strategies=True)
    if r['status']=='certified_numeric':
        if return_strategies:return r,pol
        return r
    from research.formation.schedule_v14 import solve
    ans,strategies,structures=solve(A,base,T,S,'C',time_limit=max(1,time_limit-r['seconds']),return_strategies=True)
    ans.update({'algorithm':'64-step contingent-policy generation then full sequence-form fallback',
                'pricing_initial_status':r['status'],'pricing_initial_gap':r['gap'],
                'pricing_initial_seconds':r['seconds'],'pricing_iterations':r['iterations'],
                'seconds':time.perf_counter()-started})
    # Full sequence-form solution is retained as a realization plan; do not
    # falsely label the earlier restricted policy mixture as the final one.
    policy={'representation':np.array('sequence_form'),'x':strategies['x'],'y':strategies['y'],
            'S':np.asarray(S,dtype=int),'S_R':np.array([],dtype=int)}
    if return_strategies:return ans,policy
    return ans
