...
def decompose(A, base, T, S, S_R, time_limit=1200., pricing=False):
    ...
    for t, end in reversed(list(zip(boundaries[:-1], boundaries[1:]))):
        ...
        if m == 3:
            ...
            structures = None if pricing and (not sb or not sr) else (build_flow(base, base, end-t, sb), build_flow(base, base, end-t, sr))
            ...
            for i, sub in enumerate(mats):
                if pricing and (not sb or not sr):
                    ...
                    if not sr:
                        ans = hybrid(sub,base,end-t,sb,time_limit=time_limit)
                    else:
                        a = hybrid(-sub.T,base,end-t,sr,time_limit=time_limit)
