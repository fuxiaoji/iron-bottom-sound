...
class LeaderPath:
    ...
    def sample(self, q: float) -> tuple[np.ndarray, float]:
        ...
        if j + 1 >= len(self.stations):
            # sampling exactly at the current head: no following station exists,
            # so take the head position and the incoming segment direction
            d = self.stations[j] - self.stations[j - 1]
            return self.stations[j], math.degrees(math.atan2(d[1], d[0]))
...
def batch_payoff_lf(game, cands: np.ndarray, opp: np.ndarray, blue: bool,
    ...
    def heading(st, j):
        ...
        d = st[ar, jn] - st[ar, j]           # element-wise: j is per-candidate
    ...
    def sample(st, seg, M, q):
        ...
        w = q - i
        i = np.clip(i, -M, npts - 1 - M)
        j = np.clip(i + M, 0, npts - 1)
        jn = np.clip(j + 1, 0, npts - 1)
        p = (1 - w)[:, None] * st[ar, j] + w[:, None] * st[ar, jn]
