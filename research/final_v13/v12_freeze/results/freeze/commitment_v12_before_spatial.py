...
class RemainingHorizonGame:
    ...
    def solve(self):
        c=self.config
        if c.canonical and _side_bytes(self.sB,self.game.kB)>_side_bytes(self.sR,self.game.kR):
            cfg=copy.copy(c);cfg.canonical=False
            other=RemainingHorizonGame(swap_game(self.game),self.sR,self.sB,self.R,self.levels,self.cache,cfg)
            s=other.solve();s['x'],s['y']=s['y'],s['x'];s['V']=-s['V'];s['LB'],s['UB']=-s['UB'],-s['LB']
            for row in s['trace']:row['V']=-row['V'];row['LB'],row['UB']=-row['UB'],-row['LB']
            s['canonical_swapped']=True
            return s
        ...
        for it in range(1,c.max_iters+1):
            ...
            imin=int(np.argmax(eb));jmin=int(np.argmin(er));lo=float(er[jmin]);hi=float(eb[imin])
