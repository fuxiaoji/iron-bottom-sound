...
## 7. 把 v2.3 规则套回同一几何：修前/修后对照

用**同一批报文的同一批位置**重算 v2.3 的选路（视线由旧路由的分支反推：只有视觉类分支要求视线，
这一点在报告里声明为推断而非测量），得到：

| 指标 | v2.2（旧记录实际） | v2.3（同几何重算） |
|---|---|---|
| TBS 直连 | 84 | **116** |
| 转报再加密 | **55** | **0** |
| 断链（无通路） | 2 | 0 |
| 当面交办（独立媒介） | 21（记作 TBS+0） | **21** |
| 位置无法解析、无法判定 | — | 4（标记 `undetermined_no_position`，不做猜测） |
| 媒介被改变的报文 | — | **74 / 141** |
| 需要支付"基础延迟"的报文 | **79** | **0** |

**净效果：同一场仗、同一批位置，凭空产生的 79 个"延迟回合"全部消失**，
转报再加密从 55 条降到 0 条。这不是把结果调向期望——这是把两个错误的射程/路由常量改对之后的必然结果：
137/141 条报文本来就在 73 hex 的直连射程内（§1），而跨编队从来不是"跨密码体系"（§2）。

> 附注：`blinker` 在修后仍然为 0，因为本局所有超射程的情形都不存在（最大 42 hex）；
> 视觉信号分支由 IR-2 的测试 C 单独覆盖（80 hex 且有视线 → `blinker`）。
