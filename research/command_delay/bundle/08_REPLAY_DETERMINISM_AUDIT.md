...
四个种子得到**同一个** sha256：`494b6053f0106004…`（每次改动本地代理可见的链路状态都会改变这个摘要——本次因 CD8-F3/F4 的链路语义修正而改变，属预期）。因为整个局面是 `(想定, seed, options)` 的纯函数（引擎骰点由 `state.seed * 1_000_003 + state.rng_counter` 派生），跨进程一致等价于跨机器一致。
