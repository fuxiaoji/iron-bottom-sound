"""Run only G3/G4/G5 (G1/G2 already completed with F10-fixed intents)."""
import json, sys
from pathlib import Path
sys.path.insert(0, str(REPO := Path(__file__).resolve().parents[4] / "backend" / "src"))
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
from repair.gold_cases import CASES, evaluate_case, _state_for  # noqa
from iron_bottom_sound.models import Side  # noqa

OUT = Path(__file__).resolve().parents[2] / "metrics" / "gold_cases.json"
d = json.loads(OUT.read_text())
remaining = ["G3_RANGE_CONTROL", "G4_TORPEDO_CORRIDOR_DENIAL", "G5_LOCAL_FORCE_SUPERIORITY"]
for case_name, builder, scenarios in CASES:
    if case_name not in remaining:
        continue
    built = None
    for scenario in scenarios:
        for seed in range(1, 9):
            for turn in (2, 3):
                from repair.gold_cases import reach_move
